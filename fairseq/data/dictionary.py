    ...
    def save(self, f):
        ...
        if isinstance(f, str):
            ...
            with open(f, 'w', encoding='utf-8') as fd:
                return self.save(fd)
        for symbol, count in zip(self.symbols[self.nspecial:],
...
class SmileDictionary(Dictionary):

    def __init__(self, pad='<pad>', eos='</s>', unk='<unk>', go='<go>'):
        self.mask_word = '<mask>'
        self.unk_word, self.pad_word, self.eos_word, self.go_word = unk, pad, eos, go
        self.symbols = []
        self.count = []
        self.indices = {}
        # For bert index,
    ...
    def finalize(self, threshold=-1, nwords=-1, padding_factor=1):
        super(SmileDictionary, self).finalize(
    ...
    def load(cls, f=None, ignore_utf_errors=False):
        """ Load function for SMILE data.
            ...
            Ignore the file and just initialize the dictionary.
        """
        return cls()


class GeneralSmileDictionary(Dictionary):
    ...
    def __get_smile_tokens(self):
        # The tokens are generated from ZINC, 17p, QM9, PM2, logP and TOX21.
        SMILE_TOKENS = [
            'c', 'C', '(', ')', '1', 'O', '2', 'N', '=', '3', 'n', '[C@@H]',
    ...
    def finalize(self, threshold=-1, nwords=-1, padding_factor=1):
        super(GeneralSmileDictionary, self).finalize(
            threshold=threshold, nwords=nwords, padding_factor=padding_factor)
