...
from . import (FairseqEncoder, FairseqIncrementalDecoder, FairseqModel,
               PredNet, register_model, register_model_architecture)
...
class LSTMModel(FairseqModel):
    ...
    def build_model(cls, args, task):
        ...
        if args.prop_predict:
            # FP size = num_direction * num_layers * hidden_size * 2
            # here we use both hidden state and cell state as FP.
            pred_input_dim = args.encoder_hidden_size * args.encoder_layers
            pred_input_dim *= 2 if args.encoder_bidirectional else 1
            pred_input_dim *= 2
            prednet = PredNet(
        ...
        return cls(encoder, decoder, args.prop_predict, prednet, args.get_fp)

    def forward(self, src_tokens, src_lengths, prev_output_tokens):
        encoder_out = self.encoder(src_tokens, src_lengths)
        decoder_out = self.decoder(prev_output_tokens, encoder_out)
        if self.prop_predict:
            # Decipher encoder output.
            _, hidden_out, cell_out = encoder_out['encoder_out'][:3]
            fp = torch.cat([hidden_out, cell_out], -1)
            # Size(num_layers * num_directions), batch size and hidden size(x2).
            sz, bsz, hsz = list(fp.shape)
            fp = torch.transpose(fp, 0, 1)
            fp = fp.contiguous().view(bsz, sz * hsz)
            if self.get_fp:
                return fp
...
class AttentionLayer(nn.Module):
    ...
    def __init__(self, input_embed_dim, output_embed_dim):
        ...
        self.output_proj = Linear(
            input_embed_dim + output_embed_dim, output_embed_dim, bias=False)
    ...
    def forward(self, input, source_hids, encoder_padding_mask):
        ...
        if encoder_padding_mask is not None:
            attn_scores = attn_scores.float().masked_fill_(
                encoder_padding_mask, float('-inf')).type_as(
...
def base_architecture(args):
    ...
    args.encoder_hidden_size = getattr(args, 'encoder_hidden_size',
                                       args.encoder_embed_dim)
